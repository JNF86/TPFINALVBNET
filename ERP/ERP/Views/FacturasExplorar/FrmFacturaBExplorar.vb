Public Class FrmFacturaBExplorar
    Private datas As New List(Of FacturaB)
    ...
    Private Sub getList()
        datas.Clear()
        For Each rw As DataRow In FacturaBController.getList().Tables(0).Rows
            datas.Add(New FacturaB With {
            .Id = Convert.ToInt32(rw("Id")),
            .FacturaBSerie = rw("FacturaBSerie").ToString(),
...
.Total = Convert.ToDecimal(rw("Total").ToString())
        ...
        Dim q = From c In datas
                Select c.Id, c.FacturaBSerie, c.Fecha, c.Vendedor, c.Razon, c.Direccion,
                ...
                c.Cantidad5, c.Unitario5, c.Importe5, c.Total
    ...
    Private Sub getFilterList()
        ...
        Try
            Select Case ToolStripComboBox1.SelectedIndex
                ...
                Case 1
                    DataGridView1.DataSource =
                    datas.Where(Function(x) x.FacturaBSerie.StartsWith(ToolStripTextBox1.Text, StringComparison.InvariantCultureIgnoreCase)).ToList()
            End Select
