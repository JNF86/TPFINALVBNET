...
Public Class FrmFacturaA
    ...
    Private WithEvents ctrlPrint As New PrintFacturaAController()
    Private WithEvents ctrlFactura As New FacturaAController()
    ...
    Private Sub getFacturaSerial()
        ...
        For Each rw As DataRow In FacturaSerialesController.getList().Tables(0).Rows
            serial = Convert.ToInt32(rw("SerieA"))
        Next
        ' Sumarle uno.
        serial += 1
        TxtFacturaASerie.Text = String.Format("{0}", serial)
    End Sub
    ...
    Private Sub getSearchProductos()
        ...
        Else
            ...
            For Each cl As Object In q
                ...
                TxtUnidad.Text = String.Format("{0:f2}", cl.Costo)
            Next
        End If
    End Sub
    ...
    Private Sub getIVAAlicuotaImpuesto(ByRef setAlicuota As Decimal, ByRef setImpuesto As Decimal)
        ...
        For Each rw As DataRow In FiscalController.getList().Tables(0).Rows
            ...
            Dim q = From i In lst
                    Where i.Id = IIf(CmbIVA.SelectedIndex = 0, 2, 1)
    ...
    Private Sub getCondicionesIVA()
        CmbIVA.Items.Clear()
        CmbIVA.Items.AddRange({"", "RESPONSABLE INSCRIPTO"})
    ...
    Private Sub calcTotal()
        If DataGridView1.RowCount > 0 Then
            ...
            For r As Integer = 0 To DataGridView1.RowCount - 1
                subtot1 += Convert.ToDecimal(DataGridView1.Item(4, r).Value)
            Next
            ' Cálculos para el pie final de la Factura A.
            TxtSubTotal1.Text = String.Format("{0:f2}", subtot1)
            getIVAAlicuotaImpuesto(aliIVA, impIVA) 'Obtener valores fiscales.
            impuesto = subtot1 * impIVA / 100
            TxtImpuesto.Text = String.Format("{0:f2}", impuesto)
            subtot2 = subtot1 + impuesto
            TxtSubtotal2.Text = String.Format("{0:f2}", subtot2)
            iva = subtot2 * aliIVA / 100
            TxtIVA.Text = String.Format("{0:f2}", iva)
            total = subtot2 + iva
    ...
    Private Function validateMessages()
        ...
        If TxtFacturaASerie.Text.Trim().Length = 0 Then
    ...
    Private Sub saveInvoice()
        Dim fac As New FacturaA()
        fac.FacturaASerie = TxtFacturaASerie.Text.Trim()
        ...
        fac.Subtotal1 = Convert.ToDecimal(TxtSubTotal1.Text)
        fac.Impuesto = Convert.ToDecimal(TxtImpuesto.Text)
        fac.Subtotal2 = Convert.ToDecimal(TxtSubtotal2.Text)
        fac.Total = Convert.ToDecimal(TxtTotal.Text)
        FacturaAController.insert(fac)
    End Sub
    ...
    Private Sub previewPrinting(sender As Object, e As PrintPageEventArgs)
        ...
        e.Graphics.DrawString("N: " & RSet(TxtFacturaASerie.Text.Trim(), 10), printFont, System.Drawing.Brushes.Black, e.MarginBounds.Right - 100, yPos)
        ' Colocar la Fecha.
        ...
        ' Subtotoal 1
        e.Graphics.DrawString(LSet("Subtotal", 15), printFont, System.Drawing.Brushes.Black, e.MarginBounds.Right - 180, yPos + 500)
        e.Graphics.DrawString(RSet("$ " & TxtSubTotal1.Text, 81), printFont, System.Drawing.Brushes.Black, e.MarginBounds.Left, yPos + 500)
        ' Impuesto 
        e.Graphics.DrawString(LSet("Impuesto", 15), printFont, System.Drawing.Brushes.Black, e.MarginBounds.Right - 180, yPos + 520)
        e.Graphics.DrawString(RSet("$ " & TxtImpuesto.Text, 81), printFont, System.Drawing.Brushes.Black, e.MarginBounds.Left, yPos + 520)
        ' Subtotoal 2
        e.Graphics.DrawString(LSet("Subtotal", 15), printFont, System.Drawing.Brushes.Black, e.MarginBounds.Right - 180, yPos + 540)
        e.Graphics.DrawString(RSet("$ " & TxtSubtotal2.Text, 81), printFont, System.Drawing.Brushes.Black, e.MarginBounds.Left, yPos + 540)
        ' IVA 
        e.Graphics.DrawString(LSet("IVA", 15), printFont, System.Drawing.Brushes.Black, e.MarginBounds.Right - 180, yPos + 560)
        e.Graphics.DrawString(RSet("$ " & TxtIVA.Text, 81), printFont, System.Drawing.Brushes.Black, e.MarginBounds.Left, yPos + 560)
        ' Total 
